...
*   **Dead Connection Detection via Keep-Alive Pings (`max_missed_pings`, v1.12.0+):** Detects silent socket freezes or physical cable disconnections. When the panel fails to reply to `max_missed_pings` (default: 3) consecutive keep-alive pings (0x7E `IntegraVersion`), the worker automatically transitions to `ConnectionLost` and initiates exponential backoff reconnects. Configurable via `max_missed_pings: u32` (set to 0 to disable).
